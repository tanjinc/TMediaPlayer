...
int transcode(char* inputfile, char* outputfile)
{
    int ret;
    AVPacket packet;
    AVFrame *frame= NULL;
    ...
    if (inputfile == NULL || outputfile == NULL) {
       av_log(NULL, AV_LOG_ERROR, "Usage: <input file> <output file>\n");
        return 1;
    }
    av_log(NULL, AV_LOG_DEBUG, "in: %s, out:%s",inputfile, outputfile);
   av_register_all();
